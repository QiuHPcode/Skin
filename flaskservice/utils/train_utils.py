...
def resume_model(model, runner, checkpoint, meta, resume_optimizer=True, map_location='default'):
    ...
    runner['best_val_weight_top3'] = checkpoint['meta']['best_val_weight_top3']
    runner['best_train_loss'] = checkpoint['meta']['best_train_loss']
    runner['best_val_acc'] = checkpoint['meta']['best_val_acc']
    runner['best_val_acc3'] = checkpoint['meta']['best_val_acc3']
...
def train(model, runner, lr_update_func, device, epoch, epoches, test_cfg, meta):
    ...
    TITLE = 'Train Results'
    TABLE_DATA = (
    ('Top-1 Acc', 'Top-3 Acc', 'Mean Precision', 'Mean Recall', 'Mean F1 Score'),
    ('{:.2f}'.format(eval_results.get('accuracy_top-1',0.0)), '{:.2f}'.format(eval_results.get('accuracy_top-3',0.0)), '{:.2f}'.format(mean(eval_results.get('precision',0.0))),'{:.2f}'.format(mean(eval_results.get('recall',0.0))),'{:.2f}'.format(mean(eval_results.get('f1_score',0.0)))),
    )
    table_instance = AsciiTable(TABLE_DATA,TITLE)
...
def validation(model, runner, cfg, device, epoch, epoches, meta):
    ...
    # if eval_results.get('accuracy_top-3') > runner.get('best_val_acc3'):
    #     runner['best_val_acc3'] = eval_results.get('accuracy_top-3')
    #     meta['best_val_acc3'] = runner['best_val_acc3']
    #     if epoch > 0 and os.path.isfile(runner['best_val_weight_top3']):
    #         os.remove(runner['best_val_weight_top3'])
    #     runner['best_val_weight_top3'] = os.path.join(meta['save_dir'],
    #                                                  'Val_Epoch{:03}-AccTop3{:.3f}.pth'.format(epoch + 1,
    #                                                                                            eval_results.get(
    #                                                                                                'accuracy_top-3')))
    #     meta['best_val_weight_top3'] = runner['best_val_weight_top3']
    #     save_checkpoint(model, runner.get('best_val_weight_top3'), runner.get('optimizer'), meta)
